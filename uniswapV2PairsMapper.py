...
V2Factory = w3.eth.contract(address=database['exchanges']['UniswapV2F']['address'],
                            abi=database['exchanges']['UniswapV2F']['ABI'])
...
for pair in (list(combinations(tokenNames, 2))):
    ...
    address = V2Factory.functions.getPair(w3.toChecksumAddress(token1['address']),
...
with open('uniswapPairsMap.json', 'w') as output:
    json.dump(exchangeMap, output)
    print("Written to uniswapPairsMap.json")
